...
class StrategyBase(dict):
    ...
    def _get_pnl_series_if_same_sig(self):
        """
    ...
    def _get_pnl_if_same_sig(self, row):
        ...
        Only used in `_get_pnl_series_if_same_sig`.
        """
        if row.signal == 1:
            if row.same_sig:
    ...
    def _get_signal_from_ewls(self, exog: pd.Series) -> pd.Series:
        """
        Generate a trading signal from an exponentially weighted least squares
        regression model. The trading signal
        will be 1 if we should take a long position on the portfolio,
        ...
        the exogenous variable is the Z-score of `exog`,
        calculated over the past `window_size` 4-week periods.
        """
        # Calculate Z-score of curvature relative to its `window_size`-period
        # simple moving average
        sma = exog.rolling(window=self.window_size).agg(['mean', 'std'])
        z_score = (exog - sma['mean']) / sma['std']
    ...
    def _pred_with_fees(self, without: pd.Series, fees: float):
        """
        Unused & deprecated.
...
class Strat1B(Strat1A):
    """
    Strat1A, but we don't close our position if the signal is the same
    ...
    def __init__(
        ...
        **kw,
        ...
        super(Strat1B, self).__init__(*args, **kw)
    ...
    def get_pnl(self):
        ...
        and reopening the same position.
        """
        return self._get_pnl_series_if_same_sig()
...
class Strat1C(Strat1A):
    ...
    def get_signal(self) -> pd.Series:
        ...
        fwd = self.zcb.stack(1).swaplevel().loc['fwd'][self.tenors].shift(1)
        fwd_mean = fwd.mean(axis=1)
        return self._get_signal_from_ewls(exog=fwd_mean)
...
class Strat2B(Strat2A):
    """
    Strat2A, but we don't close our position if the signal is the same
    as last month, saving on transaction fees.
    """
    ...
    def __init__(
        ...
        **kw,
        ...
        super(Strat2B, self).__init__(*args, **kw)
...
class Strat2C(Strat2A):
    ...
    def get_signal(self) -> pd.Series:
        ...
        fwd = self.zcb.stack(1).swaplevel().loc['fwd'][self.tenors].shift(1)
        fwd_slope = fwd.iloc[:, 0] - fwd.iloc[:, 1]
        return self._get_signal_from_ewls(exog=fwd_slope)
...
class Strat3B(Strat3A):
    """
    Strat3A, but we don't close our position if the signal is the same
    as last month, saving on transaction fees.
    """
    ...
    def __init__(
        ...
        **kw,
        ...
        super(Strat3B, self).__init__(*args, **kw)
...
class Strat3C(Strat3A):
    ...
    def get_signal(self) -> pd.Series:
        ...
        return self._get_signal_from_ewls(exog=fwd_curv)
...
def main(
    ...
    # Grid searching 1A_135
    grid_search_params(
        strat_class=Strat1B,
        file_stub='./data/final_proj/strat_1A_135',
        search_params=dict(
    ...
    # Grid searching 1B_135
    grid_search_params(
        strat_class=Strat1B,
        file_stub='./data/final_proj/strat_1B_135',
        search_params=dict(
            window_size=[13, 26, 52, 102],
            sigma_thresh=[-0.5, 0, 0.25, 0.5, 1.],
        ),
    ...
    # Grid searching 2A_220
    grid_search_params(
        strat_class=Strat2A,
        file_stub='./data/final_proj/strat_2A_220',
        search_params=dict(
            window_size=[13, 26, 52, 102],
            sigma_thresh=[-0.5, 0, 0.25, 0.5, 1.],
        ),
    ...
    # Grid searching 2B_220
    grid_search_params(
        strat_class=Strat2B,
        file_stub='./data/final_proj/strat_2B_220',
        search_params=dict(
            window_size=[13, 26, 52, 102],
            sigma_thresh=[-0.5, 0, 0.25, 0.5, 1.],
        ),
    ...
    grid_search_params(
        strat_class=Strat3A,
        file_stub='./data/final_proj/strat_3A_135',
        search_params=dict(
            sigma_thresh=[-0.5, 0, 0.25, 0.5, 1.],
            window_size=[13, 26, 52, 102]
        ),
    ...
    grid_search_params(
        strat_class=Strat3B,
        file_stub='./data/final_proj/strat_3B_135',
        search_params=dict(
            sigma_thresh=[-0.5, 0, 0.25, 0.5, 1.],
            window_size=[13, 26, 52, 102]
        ),
